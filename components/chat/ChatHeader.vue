<template>
  <header
    class="flex items-center justify-between p-4 bg-background border-b-4 border-double border-border shadow-md z-20 relative animate-slide-up"
  >
    <div class="flex items-center space-x-3">
      <Icon
        name="mdi:hammer-sickle"
        class="h-6 w-6 text-primary animate-pulse"
      />
      <h1
        class="text-xl font-serif font-bold tracking-widest uppercase text-primary"
      >
        Lenin GPT
      </h1>
    </div>
      ...
      <Button variant="ghost" size="icon" as-child class="ml-2">
        <a href="https://www.github.com/gaqno" target="_blank">
          <img
            :src="user.avatar_url"
            alt="GitHub"
            class="h-8 w-8 rounded-sm border border-muted filter grayscale hover:grayscale-0 transition-all"
          />
        </a>
      </Button>
    </div>
